...
def load_models():
    ...
    try:
        ...
        st.error("❌ Models not found! Please train the model first by running MLmodel.py")
        return None, None
...
def predict_emotion(text, vectorizer, model):
    ...
    if hasattr(model, 'predict_proba'):
        ...
        confidence = (max(decision_scores) / sum(abs(decision_scores))) * 100
    else:
...
def main():
    ...
    with col2:
        if analyze_button and user_input.strip():
            with st.spinner("Analyzing your mood..."):
                # Predict emotion
                emotion, confidence = predict_emotion(user_input, vectorizer, model)
    ...
    st.markdown("""
        <div style="text-align: center; color: #666; padding: 20px;">
            ...
            <p>Emotions detected: Joy, Sadness, Anger, Fear, Love, Surprise</p>
